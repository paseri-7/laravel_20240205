...
const ClauseInner =({clauses})=>{
  ...
  function search() {
    ...
    console.log('基準日', formattedDate);

    const formData = new FormData();
    // formData.append('type', selected);
    formData.append('type', parseInt(selected, 10));
    formData.append('date', formattedDate);
    
    axios.post('/search', formData)
         .then((response) => {
            console.log('APIレスポンス：', response.data);
            // コンポーネントのstateを更新して再レンダリング
            // const clauses2 = response.data
            // サーバーからのデータが {data: Array(1)} の形式だった場合
            const dataArray = response.data.data; // response.data内のdataキーにアクセス
            // 配列の内容をコンソールに表示
            console.log(dataArray);
            setClauseInfoList(dataArray);
         })
         .catch((error) => {
    ...
    <>
      ...
      {isOpen && (
        <div className="p-4 border border-gray-500">
          ...
          <div class="flex items-center justify-center gap-1 w-100 h-28">
            ...
            <button className="w-40 h-8 mr-4 text-white px-2 py-1 bg-sky-400 rounded-3xl hover:opacity-70">最新世代表示</button> 
            <button className="w-40 h-8 mr-4 text-white px-2 py-1 bg-sky-400 rounded-3xl hover:opacity-70">全世代表示</button> 
          </div>
        </div>
      )}
